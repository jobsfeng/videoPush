...
def read_frame():
    ...
    command="ffmpeg -y -f rawvideo -vcodec rawvideo -pix_fmt bgr24 -s {}x{} -r {} -i - -c:v libx264 -pix_fmt yuv420p -preset ultrafast -f flv".format(width,height,str(fps))+rtmpUrl
    print(command)
...
def run():
    ...
    thread1 = Thread(target=read_frame,)
